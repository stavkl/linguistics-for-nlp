...
The running example throughout will be of the word בבית /'in the house'/, for which the relevant multi-tag is IN^DEF\^NN. Broadly speaking, because we only fine tune BERT and not changing the segmentation algorithm (something that would require there are two strategies, one where all the word-pieces (WPs from now) get the same tag, and another where each WP can get a different tag. 




<!--stackedit_data:
eyJoaXN0b3J5IjpbNDQ0OTAwNzkzLDE4MDA1NjQwMDAsLTExMj
